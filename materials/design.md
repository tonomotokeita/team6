...
絵柄がそろったときのみ、playerのhp,speed,powerを強化する
...
アイテムはBossが倒された判定された後にBossと座標が同じところで表示、playerが同じ座標にくると取得
...
コインを消費して、アイテムを強化する。実際はplayerのhp,speed,powerを強化する。同じものは買えないようにする。
