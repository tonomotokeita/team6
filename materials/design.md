...
ボス撃破後にスロット画面へ切り替える。
絵柄が3つ揃った場合のみボーナスを獲得できる。
ボーナスの内容はPlayerのhp、speed、powerのいずれかを強化する。
絵柄が揃わなかった場合は強化なしでゲームへ戻る。
...
Bossを倒したときにBossと同じ座標へアイテムを生成する。
Playerがアイテムに触れると取得し、効果を発動する。
取得後は画面から削除する。
...
ステージクリア後にショップを表示する。
コインを消費してPlayerを強化する。
強化内容はhp、speed、powerのいずれかとする。
一度購入した強化は再購入できないようにする。
コイン不足の場合は購入できない。
